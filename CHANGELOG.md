...
- **The animation cache is read, and every clip generator checked against it.**
  ...
  file's `animationNames` roster has to contain that path; and the cache stores
  the **position in that roster** which the runtime actually dereferences. Check
  only the first two and a clip passes both while the cache still sends it
  somewhere else — so the interesting finding is not "missing" but
  **`AnimationMismatch`**, a clip whose cached index resolves to a different
  animation than the graph names. `Core/AnimData/` reads the file, `GraphDoctor`
  gains a check over it, and the three moments a clip's animation gets chosen say
  what the cache makes of it. Everything is a warning and nothing blocks a save:
  the cache is regenerated by Nemesis/Pandora *after* the graph is edited, so it
  is stale by design for exactly as long as the normal workflow takes.

  **It is read-only, and that is a property of the build rather than a promise.**
  The round-trip emitter that proves the parser faithful lives in
  `tools/hkx-animdata` and is not compiled into the shipped assembly. It earns its
  place: the format is positional and count-prefixed, so a mis-slice does not
  fail, it *shifts*, and every answer after the shift is confidently wrong.
  Counting records proves nothing against that — parse → emit → compare does,
  because the emitter recomputes every count from the structure the parser built.
  It matches vanilla's **1,892,278 bytes exactly**, and independently reproduces
  the grammar's stated totals: 429 projects, 10,597 clips, 6,725 motions, 380
  header-only. Five fault injections require a diagnosis; two are asked only to
  reject, because a shift is often detected some distance downstream and
  demanding specific wording there would assert a coincidence. Numeric fields are
  carried verbatim as strings and never reparsed — vanilla holds both `0.014` and
  `5.96046e-008`.

  **Two things in vanilla shaped the lookup, and both are the silent-wrong-answer
  shape rather than theory.** `DefaultMale` carries `CrossBow_IdleHeld`
  (animIndex 557) *and* `Crossbow_IdleHeld` (941) — one letter's case apart,
  pointing at **different animations** — and both spellings are genuinely used:
  `CrossBow_` in six vanilla behaviour files, `Crossbow_` once, in
  `horsebehavior`. A case-insensitive lookup answers `horsebehavior`'s clip with
  the other one's animation, so `FindClip` matches case-sensitively first and
  accepts a loose match only when unique, reporting ambiguity rather than
  guessing. Separately, ten project names appear **twice** (`SmallBird01`,
  `Moth`, `CommentTrigger` and seven more, all header-only props); every pair is
  the same record, so taking the first is not a guess — and the harness asserts
  *that agreement* rather than asserting the duplicates exist, so it goes red if
  it ever stops holding.

  **A graph belongs to more than one project, so the check folds over all of
  them.** `Behaviors\0_Master.hkx` is listed by `DefaultMale`, `DefaultFemale`
  and `FirstPerson`, each with its own roster; answering from whichever came
  first would be a guess with real consequences. A clip any candidate is happy
  with is fine, a complaint needs all of them, and the message names every
  project consulted instead of only the one it quotes.

  **Silence had to stop being ambiguous.** A graph with no cache warnings and a
  graph whose cache was never located produce an identical clean report, and the
  second is not the reassurance it looks like — so the stats bar names the cache
  projects in play, and says so when a cache was found but unreadable, or lists
  no project for this behaviour. Nothing there means no cache was found. Verified
  in the running editor: vanilla `sprintbehavior` against the vanilla cache
  reports **zero** cache warnings with `🎬 DefaultFemale, DefaultMale,
  FirstPerson` in the stats bar (all 289 of `0_master`'s clip generators are in
  the cache exactly, so a clean answer there is the true one); dropping
  `MT_SprintForward` from the cache's three humanoid projects produces exactly
  one warning, naming that clip. Re-introducing the case-insensitive lookup on
  purpose turns five harness checks red.
