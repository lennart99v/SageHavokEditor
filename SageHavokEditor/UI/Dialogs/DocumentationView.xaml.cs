...
namespace SageHavokEditor.UI.Dialogs
{
    ...
    "to load the full chain.");
            ...
            AddSection("tab_debugger", "Debugger Tab",
                ...
                + "editor.\n\n"
                + "⚠ The SKSE plugin is not released yet. This needs a game-side plugin called "
                + "SkyrimBehaviorDebugger, which exists and works but has not been published — it is not "
                + "bundled with this editor and it is not on Nexus, so there is currently nothing for you "
                + "to download. If you went looking for it and found nothing, that is why. Until it ships, "
                + "🎮 Live Debug will sit waiting for a game that can never answer; see "
            ...
            AddSection("debug_setup", "Live Debugging: Setup & Connection",
                ...
                + "SkyrimBehaviorDebugger is written and working, but it has not been published — it is "
                + "not bundled with this editor, it is not on Nexus, and there is no download link to "
                + "point you at. Everything below describes a feature that works once both halves are "
                + "present; right now only the editor half is something you can obtain. 🎮 Live Debug "
                + "starts a client that retries forever, which is indistinguishable from waiting for a "
                + "game you have not launched — so if you are sitting on 🔴 Live debugger "
                + "disconnected — retrying… with Skyrim plainly running, a missing plugin is the "
                + "reason.\n\n"
                + "One limitation to know once you do have it: the plugin serves a single editor session "
                + "per game launch. If you press ⏹ Stop Debug and start again, it will not accept the "
                + "second connection — restart Skyrim.\n\n"
                ...
                + "appears, so 🔴 Live debugger disconnected — retrying… is normally a wait rather "
                + "than an error — though it is also what you see when the plugin is absent, and what "
                + "you see after a second Stop Debug (see the limitation above).\n"
