...
namespace SageHavokEditor.UI.Dialogs
{
    ...
    "to load the full chain.");
            ...
            AddSection("tab_debugger", "Debugger Tab",
                ...
                + "editor.\n\n"
                + "⚠ The game half does not exist yet. The editor ships the client side only. It "
                + "connects to an SKSE plugin named SkyrimBehaviorDebugger, and no such plugin has been "
                + "written or released — so there is nothing to install and nothing to connect to, and "
                + "the panel below will stay empty however long you leave it running. Everything this "
                + "section and the three Advanced ones describe is implemented and waiting; see "
                + "Live Debugging: Setup & Connection for what is missing and what the protocol is.\n\n"
            ...
            AddSection("debug_setup", "Live Debugging: Setup & Connection",
                "Live debugging pairs the editor with a running game. The game side would be an SKSE "
                + "plugin named SkyrimBehaviorDebugger, reading the behaviour state of the actor you "
                + "are controlling; the editor side is a client that renders it against the graph you "
                + "have open. Both would have to run on the same PC — the link is a pair of local named "
                + "pipes, not a network socket.\n\n"
                + "⚠ The plugin has not been written\n"
                + "Only the editor half of this exists. There is no SkyrimBehaviorDebugger on Nexus or "
                + "anywhere else, it is not bundled with this editor, and it is not a component you have "
                + "failed to find — it has simply never been built. 🎮 Live Debug therefore starts a "
                + "client that waits forever, which looks exactly like a game that has not launched yet.\n\n"
                + "The rest of this section documents the protocol that client speaks, because it is a "
                + "real specification rather than a plan: the two pipes, what travels each way, and when "
                + "the config is sent. If you write SKSE plugins, that is everything needed to build the "
                + "other half — the full wire format is in docs/live-debugger-protocol.md in the "
                + "editor's source repository, and the editor would need no changes to talk to it.\n\n"
